...
export default function GrowthDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (snap.exists()) {
          setCompleted(snap.data() as Record<string, boolean>);
        }
      } catch (error) {
  ...
  const toggleItem = async (id: string) => {
    ...
    if (db) {
      try {
        await setDoc(doc(db, "internal_stats", "growth_checklist"), newState, { merge: true });
      } catch (e) {
  ...
  const styles = {
    ...
    progressCard: { background: "linear-gradient(135deg, #2563eb, #1e40af)", padding: 30, borderRadius: 20, color: "white", marginBottom: 40, boxShadow: "0 10px 30px rgba(37,99,235,0.3)" },
    progressTitle: { margin: 0, fontSize: 14, opacity: 0.8, textTransform: "uppercase" as const, letterSpacing: 1 },
    progressValue: { fontSize: 48, fontWeight: 800, margin: "10px 0" },
    sectionTitle: { fontSize: 20, fontWeight: 700, color: "#334155", marginTop: 40, marginBottom: 20, borderBottom: "2px solid #e2e8f0", paddingBottom: 10 },
    card: { background: "white", borderRadius: 12, padding: 20, marginBottom: 15, display: "flex", gap: 20, alignItems: "flex-start", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.05)", border: "1px solid #e2e8f0", transition: "transform 0.2s" },
    checkbox: { width: 24, height: 24, marginTop: 4, cursor: "pointer", accentColor: "#2563eb" },
    label: { fontSize: 16, fontWeight: 600, color: "#1e293b", display: "block", marginBottom: 4 },
    desc: { fontSize: 14, color: "#64748b", lineHeight: 1.5 },
    categoryBadge: { display: "inline-block", padding: "4px 8px", borderRadius: 6, fontSize: 11, fontWeight: 700, marginBottom: 8, backgroundColor: "#e0f2fe", color: "#0369a1" }
  };
      ...
      <div style={styles.container}>
        ...
        <div style={styles.progressCard}>
          ...
          <div style={{ background: "rgba(255,255,255,0.2)", height: 8, borderRadius: 4, overflow: "hidden" }}>
            <div style={{ width: `${calculateProgress()}%`, height: "100%", background: "#4ade80", transition: "width 0.5s" }} />
          </div>
        </div>
        ...
        {["ASO (App Store Optimization)", "Web SEO (Google Search)", "AI Visibility (ChatGPT/Gemini)"].map((cat) => (
          <div key={cat}>
            ...
            {STRATEGY_ITEMS.filter(i => i.category === cat).map(item => (
              <div key={item.id} style={{...styles.card, opacity: completed[item.id] ? 0.6 : 1}}>
                <input 
                  type="checkbox" 
                ...
                <div>
                  ...
                  <span style={{...styles.label, textDecoration: completed[item.id] ? "line-through" : "none"}}>
                    {item.label}
                  </span>
